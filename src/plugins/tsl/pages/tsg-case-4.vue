<template>
    <TresCanvas v-bind="state" :renderer="createWebGPURenderer">
        <TresPerspectiveCamera :fov="60" :near="0.1" :far="2000" :position="[30, 30, 30]" :look-at="[0, 0, 0]" />
...
const showRef = ref(0.5);

onMounted(async () => {
    const tsgStr = await fetch("/plugins/tsl/shader/dissolve.tsg").then(res => res.text())
    ...
    pane.addBinding(showRef, 'value', {
        label: '显示',
        min: 0,
        max: 1,
        step: 0.01,
    }).on('change', (ev) => {
        tsgObj.uniforms.show.value = ev.value;
    });
